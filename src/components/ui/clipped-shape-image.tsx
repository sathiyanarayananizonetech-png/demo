// src/components/ui/clipped-shape-image.tsx
...
const ClippedShapeGallery = React.forwardRef<HTMLElement, ClippedShapeGalleryProps>(
  ({ mediaItems, className, ...props }, ref) => {
    ...
    return (
      <>
        <svg className="clipppy absolute -top-[999px] -left-[999px] w-0 h-0 opacity-0 pointer-events-none">
          <defs>
        ...
        >
          {itemsToRender.map((item, index) => (
            <motion.figure
              ...
              className="relative overflow-hidden bg-slate-100 dark:bg-slate-900 group cursor-pointer"
            >
              {item.type === 'image' ? (
                <img
                  src={item.src}
                  alt={item.alt}
                  className="transition-transform duration-700 ease-out aspect-4/5 min-h-full align-bottom object-cover group-hover:scale-110 w-full"
                />
              ) : (
                <video
                  autoPlay
